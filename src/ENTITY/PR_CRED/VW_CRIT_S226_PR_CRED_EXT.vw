...
@tag('EXTENSION')
view VW_CRIT_S226_PR_CRED_EXT instead of VW_CRIT_S226_PR_CRED {
    type main is
	select cred(
				...
				, (select x(x.[CRED_CAT].[NAME])
					in ::[S226_CRED_CATEG] all
					where x.[CREDIT]%id = cred%id
					and [system].[OP_DATE] >= x.[DATE_BEG]
					and (x.[DATE_END] is null or x.[DATE_END] >= [system].[OP_DATE])
					and rownum = 1
				) : C_CATEGORY
